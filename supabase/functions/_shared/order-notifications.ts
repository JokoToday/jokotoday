...
function isMissingClaimRpc(error: unknown): boolean {
  ...
  return typeof candidate.message === "string" && candidate.message.includes("claim_order_notification");
}
...
export async function claimNotification(
  ...
  // endpoint with JWT + explicit owner checks. Once the RPC exists, callers
  // cannot manufacture notification work; only checkout-created events run.
  if (error && isMissingClaimRpc(error)) {
    return { mode: "legacy" };
  }
...
export async function finishNotification(
  ...
  const { data, error } = await supabase.rpc("finish_order_notification", {
    p_event_id: eventId,
    p_outcome: outcome,

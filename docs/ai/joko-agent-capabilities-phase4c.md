...
Phase 4C stores **source metadata**, not scraped source content, in its own research workspace. Source capture requires an `http` or `https` URL and an explicit source type:
...
10. Creative/operator remain unable to discover Phase 4C research-write tools.
11. Canonical Curiosity storage remains non-writable by `jokotoday`.
12. No database/RLS/Cloudflare/provider-secret/publishing change is introduced.
13. Hermes end-to-end acceptance is performed before the PR becomes merge-ready.

...
import { QrCode, Package, UserCircle, LogOut, Settings, Heart } from 'lucide-react';
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {userProfile?.profile_picture_url ? (
          <img src={userProfile.profile_picture_url} alt={userName} className="w-full h-full object-cover" />
        ) : (
          initials
        )}
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-60 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            ...
            {loyaltyPoints !== null && (
              <div
                className="mt-2.5 flex items-center gap-2 px-3 py-2 rounded-lg"
                style={{ background: 'linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%)', border: '1px solid #fde68a' }}
              >
                <span className="text-base leading-none" style={{ color: '#c6a75e' }}>★</span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-amber-700">
                    {getLabel('my_orders_page.loyalty_points_total', language, 'Loyalty Points')}
                  </p>
                </div>
          ...
          <div className="py-1">
            ...
            >
              ...
              {getLabel('likes.myLikes', language, 'My Favorites')}
            </button>
